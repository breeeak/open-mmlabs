...
#                (file_name, extension) = os.path.splitext(res)
                ...
                for shapes in data['shapes']:
                    label = str(shapes['label'])
                    if label.startswith("warp"):
                        label = "warp"
                    elif label.startswith("weft"):
                        label = "weft"
                    else:
                        continue
...
if __name__ == "__main__":
    ...
    labelme_folder = r"E:\1_dataset\fabric\test"
    #save_json_path 转换生成的coco格式的标签文件的保存路径
    save_json_path = r"E:\1_dataset\fabric\annotations/test_annotation.json"
